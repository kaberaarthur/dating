...
router.post('/register', async (req, res) => {
    const { name, email, password, phone, user_type, company_id, company_username, auth_provider = 'manual', email_verified = false, phone_verified = false, two_fa_enabled = false } = req.body;

    if (!name || !email || !password || !phone || !user_type || !company_id || !company_username) {
    ...
    try {
        ...
        const query = `
            INSERT INTO users (name, email, password, phone, user_type, company_id, company_username, auth_provider, email_verified, phone_verified, 2fa_enabled, created_at, refresh_token, token_expiry) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        ...
        const accessToken = jwt.sign({ id: userId, user_type, company_id }, JWT_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRY });

        res.status(201).json({
            message: 'User registered successfully',
            accessToken,
            refreshToken,
            user: { id: userId, name, email, phone, user_type, company_id, company_username, created_at },
        });
    } catch (err) {
        console.error(err);
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const accessToken = jwt.sign(
            { id: user.id, email: user.email, user_type: user.user_type, company_id: user.company_id },
            JWT_SECRET,
...
router.get('/profile', authenticateToken, async (req, res) => {
    try {
        const [rows] = await db.execute('SELECT id, name, email, phone, user_type, company_id, company_username, active, created_at, last_login FROM users WHERE id = ?', [req.user.id]);
